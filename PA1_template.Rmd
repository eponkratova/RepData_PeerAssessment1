...
8. Compare the average number of steps taken per 5-minute interval across weekdays and weekends.
```{r, echo=FALSE}
language <- "English" 
...
library(ggplot2)
plot4 <- ggplot(weekday)+geom_line(aes(x=interval, y=steps))
plot4
plot5 <- ggplot(weekend)+geom_line(aes(x=interval, y=steps))
plot5
require(gridExtra)
grid.arrange(plot4 + ggtitle("Weekdays"), plot5  + ggtitle("Weekend"), nrow=2)
```
